...
class ComputerGenerator(Generator):
    ...
    def generate(self, quantity: int = 1) -> None:
        computer_data = pd.read_csv(
            ComputerGenerator.ASSET_PATH, usecols=["model", "manufacturer"]
        )
        ...
        computer = pd.DataFrame(
            {
                "computer_name": super().sample_df(
                    computer_data, "model", index
                ),
                "manufacturer": super().sample_df(
                    computer_data, "manufacturer", index
                ),
                "ssd_memory": random.choices(
                    ComputerGenerator.SSD_MEMORY, k=quantity
                ),
                "hdd_memory": super().generate_list(
                    quantity,
                    lambda: random.choice(ComputerGenerator.HDD_MEMORY),
                    Generator.NULL_RATE,
                ),
                "add_info": super().generate_list(
                    quantity,
                    lambda: " ".join(self.faker.text(2000).splitlines()),
                    Generator.NULL_RATE,
                ),
