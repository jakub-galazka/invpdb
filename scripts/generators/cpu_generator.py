...
class CPUGenerator(Generator):
    ...
    def generate(self, quantity: int = 1) -> None:
        ...
        cpu = pd.DataFrame({
            ...
            "manufacturer": super().sample_df(cpu_data, "manufacturer", index),
            "cores_number": super().sample_df(cpu_data, "cores_number", index),
            "threads_number": super().sample_df(cpu_data, "threads_number", index),
            "min_clock_speed": super().sample_df(cpu_data, "min_clock_speed", index),
            "max_clock_speed": super().sample_df(cpu_data, "max_clock_speed", index),
