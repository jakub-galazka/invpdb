...
class SoftwareGenerator(Generator):
    ...
    def generate(self, quantity: int = 1) -> None:
        publishers = random.choices(
            list(SoftwareGenerator.SOFTWARES.keys()), k=quantity
        )
        software = pd.DataFrame(
            {
                "software_name": self._generate_software_names(publishers),
                "publisher": publishers,
                "license_key": super().generate_list(
                    quantity,
                    lambda: "".join(
                        random.choices(
                ...
                "expire_date": super().generate_list(
                    quantity, lambda: self.faker.date_between("-5y", "+5y")
                ),
            }
        )
